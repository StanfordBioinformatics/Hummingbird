...
def main():
    ...
    profiler = Profiler(args.profile_tool, config)
    ...
    if valid:
        print('We sugguest to test on the following machine types:')
        for row in valid:
            print(bcolors.OKGREEN + row[0] + bcolors.ENDC, str(row[1]) + 'GB')
    else:
