    ...
    for i, y in enumerate(ys):
        ...
        if len(ys) == 3:
            if i == 0:
                linestyle = ':'
                marker = 'x'
                lable = task + ' 8 threads'
            elif i == 1:
                linestyle = '--'
                marker = '^'
                lable = task + ' 16 threads'
            else:
                linestyle = '-'
                marker = 'o'
                lable = task + ' 32 threads'
        elif len(ys) == 2:
            if i == 0:
                linestyle = ':'
                marker = 'x'
                lable = task + ' 2 threads'
            else:
                linestyle = '-'
                marker = 'o'
                lable = task + ' 4 threads'
        elements = []
        regression_line, = plt.plot(x_test, regr.predict(x_test), color=colors[task], linestyle=linestyle)
        datapoint_plot, = plt.plot(x, y, marker, color=colors[task])
        ...
        plt.errorbar(target, actual_data[i], fmt=marker, color=colors[task], yerr=yerr, capsize=3, ecolor='black')
        print(plt.axis())
        xmin, xmax, ymin, ymax = plt.axis()
        offset = (ymax-ymin)*0.02*i
        plt.annotate("%.2f%%" % (100*(pred-actual_data[i])/actual_data[i]), xy=(target+0.1, (actual_data[i]+pred)/2+offset), size=11)
...
def plot_haplotypecaller():
    ...
    truth = [6236704.0, 13988248.0, 13642836.0]
    ...
    plt.savefig('haplotypecaller.png')
